...
1.  **Decoupled Intelligence**: The AI logic resides in a stateless `MLEngine` (`core/ml_engine.py`), separating complex math from the UI. This allows you to swap models (e.g., Prophet to Neural Prophet) without breaking the frontend.
2.  **Metadata-Driven Scalability**: You don't write Python code to forecast a new sales table. You simply add a row to the `analytics_config` table. The system dynamically picks up the configuration, fetches the data, trains the model, and generates alerts.
3.  **Security-First Design**:
    *   **RBAC**: Built-in Role-Based Access Control separates "Admins" (who can edit data) from "Viewers".
    *   **SQL Injection Guardrails**: The Conversational Analytics engine uses regex validation to block destructive commands (`DROP`, `DELETE`) generated by LLMs.
    *   **Safe Write-Back**: The Data Steward interface uses a transactional `MERGE` (Upsert) strategy, ensuring no data is lost during manual edits—a common pitfall in lesser systems.
4.  **Hybrid Compute**: Heavy aggregations happen in SQL Server (fast, scalable), while complex logic (Clustering, Isolation Forests) happens in Python. This prevents Out-Of-Memory errors on large datasets.

### Core Components

#### 1. 💬 Conversational Analytics (Natural Language to SQL)
*   **Function**: Allows non-technical users to ask questions like "Why did sales drop in Texas last week?"
*   **Mechanism**: Integrates with OpenAI (GPT-3.5/4). It feeds the database schema (table/column names) to the LLM as context, generates a valid SQL Server query, validates it for safety, executes it, and visualizes the result.
*   **Setup**: Requires `OPENAI_API_KEY` in `.env`.

#### 2. 🤖 Predictive Insights & Anomaly Detection
*   **Function**: Automatically detects outliers and forecasts future trends.
*   **Models**:
    *   **Anomaly Detection**: Uses **Isolation Forest** (Unsupervised) to flag data points that deviate significantly from the norm.
    *   **Forecasting**: Uses **Prophet** (if installed) or robust **Holt-Winters Exponential Smoothing** (via `statsmodels`) to handle seasonality. Falls back to Linear Regression for simple trends.
*   **Workflow**: Configured via the `Configuration Manager` page. Results are saved to `analytics_predictions` for historical tracking.

#### 3. 🧩 Clustering & Segmentation
*   **Function**: Uses **K-Means Clustering** to automatically group similar entities (e.g., "High Value Customers", "Churn Risk").
*   **UX**: Users select a table, and the system automatically identifies numeric features, runs the algorithm, and visualizes the clusters in 2D space.

#### 4. 🛡️ Data Steward (Safe Write-Back)
*   **Function**: A spreadsheet-like interface for fixing data quality issues in Staging or Dimension tables.
*   **Safety**: Unlike typical "Delete & Replace" implementations, this uses a **SQL MERGE** statement. It identifies changed rows by Primary Key and performs atomic Updates/Inserts, preserving data integrity.

#### 5. 🩺 Data Observability
*   **Function**: Monitors the health of the pipeline itself.
*   **Metrics**: Tracks Schema Drift events, Data Freshness (time since last load), and Lineage dependencies.

### Developer Workflows

#### Enabling AI on a New Table
To start generating anomalies and forecasts for a new dataset, you do not need to deploy code.

1.  **Navigate**: Go to the **Configuration Manager** page in the UI.
2.  **Select**: Choose your target table (e.g., `sales_fact`).
3.  **Map**: Select the Date Column (`order_date`) and Value Column (`total_amount`).
4.  **Activate**: Choose "Anomaly Detection" or "Forecast" and click Activate.
    *   *Behind the Scenes*: This inserts a record into `analytics_config`. The `run_predictive_analytics` asset will pick this up on its next run.

#### Extending the ML Engine
To add a new model (e.g., XGBoost):
1.  Open `elt_project/core/ml_engine.py`.
2.  Add a new static method (e.g., `train_xgboost`).
3.  Update `analytics.py` to call this method when `model_type == 'xgboost'`.

#### Configuring OpenAI
1.  Get an API Key from platform.openai.com.
2.  Add it to your `.env` file: `OPENAI_API_KEY=sk-...`
3.  The system automatically detects the key and enables the "Conversational Analytics" tab.
